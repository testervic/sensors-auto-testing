...
public class PlatformConfigurationTest {
    ...
    //配置-平台配置
    @Test(priority = 1)
    public void PlatformSetting() throws IOException, InterruptedException {
        LoginTest leLogin = new LoginTest(this.driver);
        leLogin.leLogin();
        PublicPage platformSettingCheck = new PublicPage(this.driver, "/le/Configuration/PlatformConfiguration/PlatformSetting.yaml");
        Reporter.log("<b><h2>CASE：" + platformSettingCheck.CaseName() + "</h2></b>");
        platformSettingCheck.operate();
        Assert.assertTrue(platformSettingCheck.checkpoint(this.browserType), "检查点不通过");
    }
